...
void Schedule::printTaskDetails() {
	...
	for (i=0; i<this->taskCount; i++) {
		...
		padStr(width, taskDetails[i][0]);  // print task name
...
void Schedule::printSched() {
	...
	//    taskDetails[x][y] where x=taskNum; y=0 for task name; y=1 for task's time interval.
	for (i=0; i<this->taskCount; i++) {
		padStr(width, taskDetails[i][1]);  // print task time interval
		padStr(width, taskDetails[i][0]);  // print task name
		if (i != taskCount-1) {
...
// It dynamically creates a 2D string and integer array capable of 
// storing 30 tasks and 2 details for each task (30 rows and 2 columns). 
// Said string array will be pointed to by taskDetails and taskNumbers.
void Schedule::createDynamicTaskArrays() {
	this->taskDetails = createDynamicStrArray2D(30, 2);
